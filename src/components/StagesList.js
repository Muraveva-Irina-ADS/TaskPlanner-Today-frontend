...
const StagesList = ({ stages: initialStages, onReorder, onDelete, onStage }) => {
  ...
  const handleDrop = (e, dropIndex) => {
    ...
    const updatedStages = newStages.map((stage, idx) => ({...stage, order_stage_in_list: idx + 1}));
    setDraggedItem(null);
    setDragOverIndex(null);
    if (onReorder) {
      onReorder(updatedStages.map(stage => ({id: stage.stage_id, order_stage_in_list: stage.order_stage_in_list})));
    }
  };
  //Отображается диалоговое окно подтверждения удаления проекта и при согласии администратора вызывает функцию onDelete
  const confirmDelete = (e, stage) => {
    e.stopPropagation();
              ...
              <div className="project-header">
                ...
                <div className="header-right">
                  ...
                  <Button variant="primary" className="btn-delete"
                    onClick={(e) => confirmDelete(e, stage)}>Удалить</Button>
                </div>
              </div>
