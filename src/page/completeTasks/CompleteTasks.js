...
const AppCompleteTasks = () => {
    ...
    //Получение информации о задачах со статусом "завершение"
    ...
    //Группирует задачи по проектам. В качестве ключа используется project_name пользователя, а значение — массив его задач
    ...
    //Вызов API-функции для создания задачи, являющейся полной копией завершенной задачи, но имеющией статус "ожидание"
    ...
    //Удаляет дубликаты задач по task_id, оставляя только уникальные записи
    const getUniqueTasks = () => {
        if (!tasks || !Array.isArray(tasks)) return [];
        
        return tasks.reduce((unique, task) => {
    ...
    //Восстанавливает задачу: создаёт её полную копию после подтверждения пользователя
    const handleRecoverTask = (e, task) => {
        e.stopPropagation();
        if (window.confirm(`Будет создана полная копия задачи ${task.task_name}. Продолжить?`)) {
            Copy(task.project_id, task.task_id);
        }
    };
                    ...
                    <div className="projects-grid">
                        {Object.values(tasksByProject).map((projectGroup) => (
                            <React.Fragment key={projectGroup.projectName}>
                                ...
                                {getUniqueTasks(projectGroup.tasks).map((task) => (
                                    //Карточка задачи
                                    <div key={task.task_id} className="project-card" onClick={() => handleTaskClick(task.project_id, task.task_id)}>
                                        <div className="project-header">
                                            ...
                                            <div className="header-right">
                                                ...
                                                <Button variant="primary" className="btn-recover" onClick={(e) => handleRecoverTask(e, task)}>
                                                    Восстановить
                                                </Button>
                                                <Button variant="primary" className="btn-delete" onClick={(e) => handleDeleteTaskClick(e, task)}>
                                                    Удалить
                                                </Button>
                                            </div>
                                        </div>                
