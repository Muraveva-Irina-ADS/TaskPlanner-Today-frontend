...
import {get_settings, get_users, get_matrix, get_status, get_admin_settings_note, get_admin_matrix_note, 
    get_admin_status_note, user_post, user_put } from "../../api/commonApi";
...
const AppUsers = observer(() => {
    ...
    //Получение информации о настройках всех пользователей
    ...
    //Получение информации о настройках матрицы всех пользователей
    ...
    //Получение информации о настройках статусов всех пользователей
    ...
    //Хук useEffect для фильтрации списка пользователей в массив filteredUsers
    useEffect(() => {
    ...
    const openModal = (userData = null, modalType = null) => {
        if(userData){
            ...
            setFormData({...userData, ...userSettings, role_name: 'user', matrix: userMatrix, status: userStatus,});
            setSelectedUser(userData);
            setIsNewUser(false);            
        } else {
            setSelectedUser(null);
            setFormData({...adminSettings, matrix: adminMatrix, status: adminStatus});
            setIsNewUser(true);
        }
    ...
    //Возвращает массив заголовков колонок таблицы в зависимости от выбранных администратором видимых полей
    ...
    //Валидация полей формы и вызов функции для добавления или изменения данных пользователя
    const handleUser = () => {
        const userData = {
            user: {
    ...
    //Вызов API-функции для добавления нового пользователя
    ...
    //Вызов API-функции для изменения данных пользователя
    ...
    const getFormattedValue = (user, key, userSettings) => {
        let value;
        ...
        <div className="project-wrapper">
            ...
            <div className="project-layout">
                ...
                {modalType === 'user_modal' && (<ModalStr show={showModal} onHide={closeModal} modalType={modalType} title={isNewUser ? 'Добавление нового пользователя' : 'Редактирование настроек пользователя'}
                    formData={formData} onChange={handleChange} onSave={handleUser} error={error} isNew={isNewUser} fields={['first_name', 'last_name', 'email',
                    'curPassword', 'newPassword', 'role_users_name', 'phone_number', 'birthday', 'note', 'title_settings', 'limit_tasks', 'pomodoro_duration', 
                    'rest_duration', 'start_working_day', 'end_working_day', 'number_pomodoro_per_day', 'title_matrix', 'matrix_put', 'title_status', 'status_put']} 
                    extraData={{user: user, selectedUser: selectedUser, adminSettings: adminSettings, adminMatrix: adminMatrix, adminStatus: adminStatus}}/>)}
                    {/*Кнопка Назад*/}
                    ...
                    {/*Блок с кнопками для добавления пользователя и открытия страниц с информацией о проектах, задачах и этапах пользователей*/}
                    ...
                    <div className="users-table-container">
                        ...
                        <tbody>
                            {filteredUsers.map((user) => {
                                ...
                                const showMatrix = visibleColumns.matrix_part || visibleColumns.matrix_name || visibleColumns.description || visibleColumns.color;
                                    ...
                                    <React.Fragment key={user.id}>
                                        <tr className="user-main-row">
                                            {getColumnKeys().map((key, colIndex) => (
                                                <td key={colIndex} className="user-data-cell">{getFormattedValue(user, key, userSettings)}</td>
                                            ))}
