...
const AppAuth = observer(() => {
    ...
    //Валидация почты и вызов API-функции для авторизации пользователя, получения и сохранения JWT, а также проверка, необходима ли смена пароля 
    const click = async () => {
        setError('');
        if (!email || !password) {
        ...
        <Container className="auth-container" >
            <Card className="auth-card p-5">
                ...
                <Form className="d-flex flex-column">
                    ...
                    <Form.Control className="mt-3 custom-input" placeholder="Введите ваш email..."
                        value={email} onChange={e => setEmail(e.target.value)} type="email"
                    /></div>
                    <div><label className="d-block">Введите пароль</label>
                    <Form.Control className="mt-3 custom-input" placeholder="Введите ваш пароль..."
                    ...
                    <Row className="d-flex justify-content-between mt-3 pl-3 pr-3 auth-links">
                        <div className="auth-text">Нет аккаунта? <NavLink to="/registration" className="auth-link">Зарегистрируйся!</NavLink></div>
                    </Row>
                    <div className="button-container"><Button variant={"outline-success"} onClick={click}>Войти</Button></div>
                </Form>
            </Card>
        </Container>
    );
