...
const AppProject = observer(() => {
    ...
    const handleAddProject = async() => {
        ...
        const data = await project_post(formData.project_name, formData.description, formData.color, formData.is_active);
            if (data) {
    ...
    return (
        <div className="project-wrapper">
            ...
            <Navigate/>
            <ModalStr show={showModal} onHide={closeModal} modalType={modalType} title={'Создание проекта'}
                        formData={formData} onChange={handleChange} onSave={handleAddProject} error={error} isNew={true}
                    ...
                    <div className="daily-plan-block">
                        ...
                        <div className="table-header">
                            ...
                            <div className="col" style={{ position: 'relative' }} title = 'Информация о количестве помидоров задачи'>
                                <HelpTip>
                                    В этом столбце можно увидеть количество помидоров, которое уже было затрачено на задачу или этап и сколько помидоров Вы запланировали потратить<br/>
                                    В конце идет подсчет помидоров, причем уже потраченные помидоры учитывают все помидоры, даже те, у которых не была выбрана задача или этап 
                                </HelpTip>
                                Помидоры</div>
                        ...
                        {tasks && Array.isArray(tasks) && tasks.length > 0 ? ((() => {
                            ...
                            return uniqueTaskIds.map(taskId => {
                                ...
                                return (
                                    <React.Fragment key={taskId}>
                                        <div className="table-row" onClick={() => handleTaskClick(task.project_id, task.task_id)}>
                                            <div className="col">
                                                <span className={`task-status-badge`} style={{backgroundColor: task.task_exec_color}} title = "Статус выполнения">{task.task_exec_status_name}</span>
                                            ...
                                            <div className="col">
                                                <span className="task-time" title = 'Сколько уже было помидоров / запланированное количество помидоров на задачу'>
                                                    ...
                                                    {(() => {
                                                        const todayPomodoro = pomodoroWithoutTasks.grouped?.find(p => p.task_id === task.task_id && p.stage_id === 0);
                                            ...
                                            <div className="col">
                                                <span className="task-time" title = 'Сколько уже было помидоров / запланированное количество помидоров на задачу'>
                                                    <Button variant="outline-light" size="sm" onClick={(e) => {e.stopPropagation(); navigate(`/project/${task.project_id}`);}} style={{ margin: '0px' }} title="Перейти в проект задачи">Перейти</Button>
                                                </span>
                                            </div>
                                            ...
                                            <div className="stages-table">
                                                {stages.map((stage, idx) => (
                                                    <div key={idx} className="table-row" onClick={() => handleStageClick(task.project_id, task.task_id, stage.stage_id)}>
                                                        <div className="col">
                                                                <span className={`task-status-badge`} style={{backgroundColor: stage.stage_exec_color}} title = "Статус выполнения">{stage.stage_exec_status_name}</span>
                                                        ...
                                                        <div className="col">
                                                            <span className="task-time" title = 'Сколько уже было помидоров / запланированное количество помидоров на этап'>
                                                                ...
                                                                {(() => {
                                                                    const todayPomodoro = pomodoroWithoutTasks.grouped?.find(p => p.stage_id === stage.stage_id);
                                                                    const count = todayPomodoro?.count || 0;
                                                                    return count > 0 ? ` (+${count} за ${getDateTitle(selectedDate)})` : '';
                                                                })()}
                                                            </span>
                                ...
                                {tasks.length > 0 && (<>
                                        <div className="table-header">
                                            <div className="col">
                                                <span className="stats-label">Выполнено задач и помидоров:</span>
                                            </div>
                                            <div className="col">
                                            <span className="stats-value">Задач: </span>
                                                <span className="stats-label" title = 'Выполненные задачи / Всего задач на указанный день'>
                                                    {(() => {const uniqueTaskIds = [...new Set(tasks.map(t => t.task_id))];
                                            ...
                                            <div className="col">
                                                ...
                                                {(() => {
                                                    ...
                                                    const totalSpent = Number(uniqueTasks.reduce((sum, t) => sum + (t.tasks_pomodoros_spent || 0), 0)) + Number(tasks.reduce((sum, t) => sum + (t.stages_pomodoros_spent || 0), 0));
                                                    const totalPlanned = Number(uniqueTasks.reduce((sum, t) => sum + (t.tasks_pomodoros_planned === -1 ? 0 : t.tasks_pomodoros_planned || 0), 0)) + Number(tasks.reduce((sum, t) => sum + (t.stages_pomodoros_planned === -1 ? 0 : t.stages_pomodoros_planned || 0), 0));
                                        ...
                                        <div className="table-header">
                                            <div className="col">
                                                <span className="stats-label">Прогресс помидоров:</span>
                                            </div>
                                            <div className="col">
                ...
                <div className="project-header">
                    ...
                    <div className="header-right"> 
                        <span className={`project-status ${project.is_active ? 'active' : 'inactive'}`}>
                            {project.is_active ? 'Активен' : 'Не активен'}
                        </span>
                        <Button variant="primary" className="btn-delete" onClick={(e) => {e.stopPropagation(); 
                            if (window.confirm(`Проект ${project.project_name} будет удален без возможности восстановления. Продолжить?`)) {
                                handleDelete(project.id);}}}>Удалить</Button>
                    </div>
                </div>                
